...
After adding the advise with =ox-html-markdown-style-footnotes-add=, the exported footnotes are Markdown-style unordered lists:
...
#+begin_src shell
  emacs --batch \
        ...
        --eval '(setq org-html-preamble nil)' \
        --eval '(setq org-html-postamble nil)' \
        --eval '(ox-html-markdown-style-footnotes-add)' \
        --funcall org-html-export-to-html
  cat /tmp/footnote-2.html | tail -n13 | head -n10
#+end_src

#+RESULTS[306192f3e02337f9cbe11bae5b44bb372aa1ba22]:
#+begin_src html
<hr>
<ol>
